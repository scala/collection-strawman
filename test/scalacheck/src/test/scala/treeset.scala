...
object TreeSetTest extends Properties("TreeSet") {
  ...
  property("to(Seq)") = forAll { (elements: List[Int]) =>
    val subject = TreeSet(elements.toClassic: _*)
    elements.distinct.sorted == subject.to(Seq)
  }
  ...
  property("slice") = forAll(genSliceParms) { case (subject, from, until) =>
    ...
    slice.size == until - from && subject.to(Seq) == subject.take(from).to(Seq) ++ slice ++ subject.drop(until)
  }
  ...
  property("remove single") = forAll { (subject: TreeSet[Int]) => subject.nonEmpty ==> {
    val element = oneOf(subject.to(Seq).toClassic).sample.get
