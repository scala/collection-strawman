...
trait IterableOps[+A, +CC[X], +C] extends Any {
  ...
  /** Given a collection factory `fi`, convert this collection to the appropriate
    ...
    */
  def to[C1](f: FromSpecificIterable[A, C1]): C1 = f.fromSpecificIterable(iterable)
  ...
  /** Returns a new $coll containing the elements from the left hand operand followed by the elements from the
    ...
    */
  def concat[B >: A](xs: IterableOnce[B]): CC[B] = fromIterable(View.Concat(iterable, xs))
  ...
  /** Returns a $coll formed from this $coll and another iterable collection
    ...
    */
  def zip[B](xs: IterableOnce[B]): CC[(A @uncheckedVariance, B)] = fromIterable(View.Zip(iterable, xs))
